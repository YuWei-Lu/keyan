	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(method.equals("adddw")){  // 单位   ***********代码注释  
			...
			try {       
	            ...
	            // 要用来做导入用的sql目标文件：       
	            FileOutputStream fout = new FileOutputStream("c:/db.sql");       
...
////////////-----------------------------------成果
			...
			String mc = request.getParameter("mc"); 
			String bz = request.getParameter("bz");
			String jzr = request.getParameter("jzr");  
			String username=(String)session.getAttribute("user");
			String dw=cBean.getString("select address from admin where username='"+username+"'");
			int flag = cBean.comUp("insert into hd(mc,bz,jzr,sj,yh,dw)  values('"+mc+"','"+bz+"','"+jzr+"','"+date+"','"+username+"','"+dw+"')");
			if(flag == Constant.SUCCESS){ 
				request.setAttribute("message", "操作成功！");
			...
			String username=(String)session.getAttribute("user");
			String dw=cBean.getString("select address from admin where username='"+username+"'");
			int flag = cBean.comUp("update hd set mc='"+mc+"',bz='"+bz+"','"+jzr+"',sj='"+date2+"',yh='"+username+"',dw='"+dw+"' where id='"+id+"'");
			if(flag == Constant.SUCCESS){ 
				request.setAttribute("message", "操作成功！");
				request.getRequestDispatcher("admin/hd/index.jsp").forward(request, response);
			}
