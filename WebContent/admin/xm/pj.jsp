...
function top2(){
   	form3.action="<%=basePath%>admin/xm/pj.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>admin/xm/pj.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/xm/pj.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/xm/pj.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/xm/pj.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/xm/pj.jsp?page="+page;
      form3.submit();
...
<div class="title_right"><strong>项目评价管理</strong></div>  
...
<%   
	...
	int cou = cb.getMessageCount("select count(*) from xm where zhj='是'");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from xm where zhj='是' order by id desc",18);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap"> 
         <a href="<%=basePath%>admin/xm/pjadd.jsp?method=pjxm&id=<%=pagelist2.get(0).toString()%>">评分</a>
