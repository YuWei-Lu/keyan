...
function top2(){
   	form3.action="<%=basePath%>admin/cg/s2info.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>admin/cg/s2info.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/cg/s2info.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/cg/s2info.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/cg/s2info.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/cg/s2info.jsp?page="+page;
      form3.submit();
...
<table class="table table-bordered table-striped table-hover">
     <tbody>
       <tr align="center">
         ...
         <td nowrap="nowrap"><strong>成果类别</strong></td> 
         <td nowrap="nowrap"><strong>成果名称</strong></td> 
         <td nowrap="nowrap"><strong>成果申报人</strong></td>
         <td nowrap="nowrap"><strong>成果申报时间</strong></td>
         <td nowrap="nowrap"><strong>成果说明</strong></td>
         <td nowrap="nowrap"><strong>成果支撑材料下载</strong></td>
...
<%	String word=Common.toChineseAndTrim(request.getParameter("word")); 
	...
	int cou = cb.getMessageCount("select count(*) from cg where bh like '%"+word+"%' and dw='"+dw+"'");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from cg where bh like '%"+word+"%' and dw='"+dw+"' order by id desc",13);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap">
         <a href="<%=basePath%>admin/cg/sp.jsp?method=spcg&id=<%=pagelist2.get(0).toString()%>">审批</a> 
         <a href="<%=basePath%>ComServlet?method=delcg2&id=<%=pagelist2.get(0).toString()%>">删除</a>
         </td>
       </tr>
<% }} %>
       <tr align="center">
